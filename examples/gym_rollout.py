...
setup runs once; every episode restores the task's filesystem checkpoint):
...
The reference Docker provider uses the hardened filesystem tier: each reset starts a
fresh container from the immutable checkpoint image. The historical live warm-pool graft
is intentionally disabled because it could race guest writers.
"""
