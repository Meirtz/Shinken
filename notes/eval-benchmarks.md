...
| `eval()` on guest/file strings; `is_utc_0` parses `timedatectl` line index 3 | A non-numeric `result.txt` silently scores 1.0 | Typed assertion nodes; no `eval()` of guest-returned strings |
...
| **D2 / D3** (ACI + observation) | Verifiers introspect via the ACI over stable channels (one cross-OS abstraction, no host-side path branching); a11y + Set-of-Marks observation on top of screenshot baseline attacks the 23%-of-errors grounding problem and cuts grading-evidence bandwidth where coverage is strong. One neutral Operator scaffold holds the harness constant. |
| **D6** (permissions) | Explicit agent-vs-grader separation; capability scoping per eval; map OpenAI `pending_safety_checks` / Anthropic HITL onto the permission panel so the vendor approval hooks work out of the box. Approvals/denials are first-class replay events. |
