...
record the whole run as **`.skn` replay/data**, move task artifacts, declare and record sandbox
...
| E8 | A tiny eval harness runs deterministic tasks repeatedly and emits verifier receipts linked to `.skn` bundles. | Eval evidence (D7) exists before cloud-scale eval service. |
| E9 | Contract tests cover ACI schema, Rust protocol, Python SDK, adapters, `.skn`, capability events, and verifier receipts. | v0.0.1 cannot regress into schema/runtime drift. |
...
- **Acceptance:** record an M1 session; replay re-renders the action timeline + observation states;
  ...
  semantics are not confused with runtime checkpoint/restore.
...
- **Realizes:** D6 (capability + audit, v0), D7 (verifier-first eval, v0). Hits E2/E3/E4/E8.
