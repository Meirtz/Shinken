...
timeline
    ...
    M2 : .skn record + replay (audit ledger)
    Runtime state : snapshot/checkpoint/fork/resume + Docker disk tier
...
### M2 — `.skn` record + replay (the audit/training ledger)
...
### M2c — runtime state: snapshot/checkpoint/fork/resume (the differentiator)
- **Build:** provider capability descriptors for `snapshot`, `checkpoint`, `fork`, `restore`,
  ...
  implementation** of them — `docker commit` snapshot → restore/resume/fork and an
  event-offset-bound `checkpoint` that binds a substrate snapshot to a `.skn` event offset
  (`snapshot_kind="disk"`, #206/#209/#211). External providers report provider-managed behavior; no
  provider may imply Firecracker-class **memory** fork unless code and tests prove it. This is the
  headline runtime primitive — replay records *what happened*, a checkpoint says *where this can
  continue from* (the checkpoint references the replay offset, not the reverse).
- **Acceptance:** SDK/provider contract tests verify unsupported operations fail explicitly *and*
  that the Docker disk tier round-trips snapshot/restore/fork/resume; `sandbox.checkpoint()` records
  a `snapshot_ref` event at the correct `.skn` offset (#211); docs/user runtime-state explains the
  terms and tiers honestly.
- **Realizes:** D1 (CoW fork-from-snapshot), D5 (checkpoint/fork semantics + `.skn` link), D7 (N≥5
  forked eval replicas), D9 (scheduler inputs).
