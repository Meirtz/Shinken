...
- **Error taxonomy (v0, #56):** wire errors distinguish infrastructure death from agent/task failure. Minimum: a `sandbox_died` error class carrying exit/signal detail (the -9 SIGKILL/OOM idiom) so eval/RL consumers can branch retry-with-more-resources vs mark-trajectory-failed; a typed per-action status (`ok | timeout | error | skipped`); and a trajectory-level exit reason with documented precedence. Failures yield structurally valid (masked) records — a failed step must never crash a batch. (Prior art: uni-agent's three-level ToolStatus/exit_reason/traj_masked taxonomy — https://github.com/verl-project/uni-agent; Agentix's `WorkerExited` carrying the worker returncode — https://github.com/Agentix-Project/Agentix.)
- **Screencast delivery semantics (#56):** alongside frame-queue bounding, specify reconnect behavior for `stream`/`seq`: client emits a resume with pending ids on reconnect, server replays unacked events, client acks — at-least-once for the event stream, freshest-wins (drop-stale) for frames. (Working reference: Agentix's resume/ack reliable-stream design — https://github.com/Agentix-Project/Agentix.)
- **Scorer isolation (eval, #206/#231):** a task verifier runs in a fresh subprocess, receives its params as JSON on stdin, and writes its verdict to a result file via atomic write+fsync+rename; the parent treats a written result file as authoritative even if the child exits non-zero or times out after writing. This neutralizes noisy third-party evaluator code (stray stdout, non-zero shutdown exit codes) corrupting a score, and is the form `run_eval_forked`'s scorer takes when scoring a forked state.
- **Infra-vs-task failure split (eval):** the harness sets an `agent_started` marker only after the agent loop begins; an exception BEFORE it propagates as infrastructure failure (retry on a fresh sandbox/fork), an exception AFTER it yields score=0 + a recorded error (a task failure that must NOT consume infra retries). This pairs with the `sandbox_died` error class (#56) so eval/RL consumers branch retry-with-more-resources vs mark-trajectory-failed correctly.
