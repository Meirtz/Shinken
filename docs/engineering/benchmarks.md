...
- **Figures** (tracked, embedded below): [`docs/assets/bench/*.png`](../assets/bench),
  ...
  `python benchmarks/replot.py`. Regenerating figures needs only `matplotlib` and `seaborn`
  (no TeX: the shared style renders serif text with matplotlib's own engine, so `replot.py`
  runs on any contributor machine).
...
(base64+JSON framing adds ~33% on TEXT sessions; binary WS frames — the negotiated default — carry payloads at ~1.006× wire/payload, measured in §3). Each suite boots fresh sandboxes through
...
- **Bytes/step is honestly mixed and favors OSWorld at default codecs.** OSWorld's PNG of
  ...
  text-legibility envelope; the delta screencast's ~2.4 KiB/frame in §3) and the BUILT, negotiated-default
  binary frames already remove the 33% (measured wire/payload 1.006×, §3); the **default-PNG encode density itself is a real, open
