...
remains unique; the capability/resource-scoping plumbing stays a minor runtime seam (not a headline);
the `.skn` deferral (#216) remains correct and is *reinforced* by trajectory export being commodity.
...
| F‑9 | Python `inject.py` | injector `args` shlex-quoted; readiness poll after detached start (injection used to "succeed" even if the binary never started); ssh argument-quoting guard so a stray arg can't alter the remote command. | ✅ P1 |
...
## 5. Testing / CI / ops / runtime hardening
...
| T‑4 | Docker image pinned to a single Debian release (bookworm) across both build stages and run as a **non-root** user; tightened run-flags documented (`--cap-drop ALL`, `--security-opt no-new-privileges`, `--read-only`); dependabot gained a `docker` ecosystem. | Floating `rust:1-slim`/`debian:stable-slim` could drift glibc across stages; running as a non-root user keeps the container's footprint bounded. | `images/linux/Dockerfile`, `.github/dependabot.yml` | ✅ P1 |
...
| T‑7 | Runtime boundary hardening: timing-safe token compare, pre-auth deadline, 16 MiB inbound cap, token required on every TCP listener, browser-Origin deny-by-default with an exact allowlist, and in-guest `exec` default-off. The full production Capability Manager / per-action resource-scoping layer remains designed-only. | The agent-to-host boundary is the load-bearing one for a sandbox running arbitrary agent actions. | `shinkend/`, [isolation & capability note](../design/threat-model.md) | ✅ P0 |
