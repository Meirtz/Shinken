...
| A‑2 | **Token fidelity is an adapter requirement** recorded: when collecting against a token-level inference server, adapters must pass through token ids/masks/logprobs — messages-only records are lossy for RL (retokenization mismatch). | The train lane (#223) is unusable for RL without this; two reference stacks make it explicit. | [`../design/agent-runtime.md`](../design/agent-runtime.md) §5.1 | ✅ P1 |
...
| C‑4 | **#56 contract reconciliation + hardening** — the screencast wire vocabulary is now schema-validated and contract-tested (was the open #56 gap); the remaining #56 work is the **error taxonomy** (a `sandbox_died` class with exit/signal detail, a typed per-action status, a trajectory-level exit reason) and **screencast reconnect semantics** (`stream`/`seq` resume/replay-unacked/ack: at-least-once events, freshest-wins frames). | RL/eval consumers must branch infra-death vs task-failure and need a specified reconnect contract for the stream fields the runtime already ships. | [`v0.0.1-plan.md`](v0.0.1-plan.md) §6 | 🟡 P1 |
...
| T‑5 | **Scorer isolation** (subprocess + atomic result-file, result-authoritative-over-exit-code) and the **infra-vs-task failure split** (`agent_started` marker) recorded as the form the eval/`run_eval_forked` scorer takes. | A noisy third-party evaluator must not corrupt a score, and an eval flake must not consume infra retries. | [`v0.0.1-plan.md`](v0.0.1-plan.md) §6 | 🟡 P1 |
...
1. **#56 error taxonomy + screencast reconnect semantics** (C‑4) — the remaining half of the contract
   hardening; spec the `sandbox_died` class, per-action status, exit-reason precedence, and the
   resume/ack stream contract. *P1.*
...
5. **Scorer isolation + infra/eval split** (T‑5) — implement in `run_eval_forked`'s scorer. *P1.*
