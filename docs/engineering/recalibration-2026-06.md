...
| T‑5 | **Infra-vs-task failure split — implemented**: `run_eval`/`run_eval_forked` classify each run (`pass\|fail\|setup\|sandbox_died\|error`), count `infra_errors` separately, and confirm sandbox death via `provider.check_alive()`. **Subprocess scorer isolation** (atomic result-file, result-authoritative-over-exit-code) is scoped to the *external-evaluator* lane (OSWorld scorer), not the in-process reference verifier. | A noisy third-party evaluator must not corrupt a score, and an eval flake must not consume infra retries. | `eval.py`, `providers/*`; [`v0.0.1-plan.md`](v0.0.1-plan.md) §6 | ✅ (split) / 🟡 (subprocess scorer) P1 |
...
5. **Infra/eval split DONE; subprocess scorer isolation remaining** (T‑5) — the failure split +
   `provider.check_alive()` shipped; subprocess+result-file scorer isolation lands with the external
   OSWorld evaluator, not the in-process reference verifier. *P1.*
