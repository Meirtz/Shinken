...
| T‑7 | Runtime boundary hardening: timing-safe token compare, pre-auth deadline, 16 MiB inbound cap, non-loopback-without-token refusal — all already built and re-verified; remaining is the boundary② callback-path note (added) and the production Capability Manager / resource-scoping layer (designed-only). | The agent-to-host boundary is the load-bearing one for a sandbox running arbitrary agent actions. | `shinkend/`, [isolation & capability note](../design/threat-model.md) | ✅ P1 |
