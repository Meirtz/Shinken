...
> Reconciles to canon **D6** (permission/isolation), **D9** (Action Gateway / control plane), **D5** (replay), **D4** (transport), **D1** (isolation substrate). Sibling docs: [02-architecture](02-architecture.md) · [05-tech-decisions](05-tech-decisions.md#d6--permission-is-3-layer-capability-unlock-cedar--ocap--os) · [06-roadmap](06-roadmap.md) · [07-glossary](07-glossary.md) · [09-economics-and-build-vs-buy](09-economics-and-build-vs-buy.md). Sources: [../notes/sources.md](../notes/sources.md).

Shinken runs **untrusted-by-default code-use and computer-use agents at ultra-high concurrency** against isolated desktop Sandboxes, brokers their privileged actions through a capability-unlock Permission Panel, and records everything as a forkable `.skn` replay. That combination makes Shinken a high-value target: a single platform that holds many tenants' credentials, network paths, and trajectories, driven by models we must assume are *jailbreakable*.
...
```
   ...
   (Operator)       │  Fleet Mgr · Scheduler · Replay store ──⑤── · Eval svc     │
                    └───────┬───────────────────────┬──────────────────┬─────────┘
                            │② vsock/ACI             │② (other tenant)  │⑥ HITL card
                    ┌───────▼───────┐         ┌───────▼───────┐    ┌────▼─────────┐
                    │ Guest Sandbox │   ③     │ Guest Sandbox │    │ Human        │
                    │  (tenant A)   │◀───X───▶│  (tenant B)   │    │ approver     │
...
| ⑥ | **Human approver** | Verified operator identity | The approval *request* (it may be injection-driven) | The human is shown actions, not the agent's narration; a denied action returns as a tool result, not a silent failure; presence for the top tier is *verified*, not assumed | Stripped-transcript action cards; identity-bound elicitation; Watch-Mode presence checks; denial circuit-breaker (3 consecutive / 20 total) |
...
STRIDE = Spoofing, Tampering, Repudiation, Information disclosure, Denial of service, Elevation of privilege. The table maps each asset to its dominant threats and the [D6](05-tech-decisions.md#d6--permission-is-3-layer-capability-unlock-cedar--ocap--os)/[D9](05-tech-decisions.md) control that breaks them.
...
| **Human approver loop** | ● | | ● | | ● | | Approval fatigue → rubber-stamp; absent-presence theater; injection-driven grind | Stripped-transcript cards; batched/scoped grants; verified Watch-Mode presence; denial circuit-breaker; fail-closed on timeout |
...
- **Deny-overrides scoped allowlist** (`None < Allow < Deny`, deny wins; deny-by-default; `host`/`*.host`/`**.host` only; reject bare global `*` in deny rules). The Permission Panel surfaces the *breadth* of each entry and warns on broad hosts ([Cloudflare Outbound Workers](https://blog.cloudflare.com/sandbox-auth/) validates this exact dynamic-egress pattern in production).
...
- **Human-fatigue circuit-breaker**: a denial threshold (3 consecutive / 20 total in a session) hard-escalates to a human or terminates a headless session, so a prompt-injected agent cannot grind the approver indefinitely; concurrent Asks coalesce; a denied action returns to the agent as a tool result instructing it to respect the boundary, not as a silent retry loop ([Claude Code auto mode](https://www.anthropic.com/engineering/claude-code-auto-mode)).
...
D6 is a **three-layer split** because no single layer does the whole job: Cedar *decides*, the capability handle is the live *switch*, and the OS is the *wall*. The Permission Panel ("unlock advanced image features") is the human seam across all three.
...
**Layer 1 — Cedar declarative decision layer.** Cedar (not OPA/Rego) is the policy engine because the unlock panel is a privilege-escalation surface and the property that matters most is being able to **prove** a grant or policy edit "never grants more than before." Cedar's SMT/Lean-verified symbolic compiler (Cedar Analysis) gives that as a pre-grant gate and in CI; Rego's Turing-flexibility cannot. Cedar's PARC + `permit`/`forbid` + forbid-overrides + deny-by-default *is* the capability-unlock semantics, runs sub-ms (vendor-published 42–60× faster than Rego, unverified), and AWS Bedrock AgentCore ships this exact pattern in ENFORCE mode ([Cedar policy structure](https://docs.cedarpolicy.com/policies/syntax-policy.html); [Cedar Analysis](https://aws.amazon.com/blogs/opensource/introducing-cedar-analysis-open-source-tools-for-verifying-authorization-policies/); [Bedrock AgentCore + Cedar](https://shinyaz.com/en/blog/2026/03/15/bedrock-agentcore-policy-cedar-authorization)). The 8 capability classes (`net.egress`, `fs.scope`, `clipboard`, `gpu`, `install.privileged/sudo`, `persistence`, `credentials`, `peripheral`) each default empty/false and compile to Cedar entities; admin `forbid` policies form the un-overridable hard-deny tier.
...
**Layer 3 — OS enforcement.** The wall that ignores model intent: Linux = bubblewrap + seccomp (network-gate + `ptrace`/`io_uring` deny) + Landlock + cgroups + the out-of-VM egress proxy; macOS = Seatbelt + TCC; Windows = restricted token + per-workspace capability-SID + WFP egress block. The panel must degrade to the *weakest* per-OS enforcement and say so honestly (e.g. Windows egress without AppContainer is coarse).
...
**Taint-tracking + Agents-Rule-of-Two / CaMeL** is the top-level architectural gate, because model defenses fail. Track three flags per turn — *untrusted-content ingestion*, *sensitive-data access*, *external communication* — and allow at most **two** unattended; if all three are needed, force human-in-the-loop on the exfil-capable action ([Lethal Trifecta](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/); [Agents Rule of Two](https://simonw.substack.com/p/new-prompt-injection-papers-agents)). Operationally: any action whose parameters derive from tainted data is auto-promoted at least one risk tier (Auto → Ask) regardless of the tool's base risk; a tool-output injection probe runs *before* the agent consumes content and its verdict appears on the approval card. CaMeL's code-then-execute taint tracking (67% of AgentDojo with provable security — [CaMeL paper](https://arxiv.org/pdf/2503.18813)) and the dual-LLM / action-selector patterns ([Willison, design patterns](https://simonwillison.net/2025/Jun/13/prompt-injection-design-patterns/)) are the spectrum of strength Shinken matches to risk tier.

**HITL — the Permission Panel.** Four risk tiers map to enforcement: **Auto** (scoped reads, loopback net — silent but logged), **Notify** (low-risk workspace writes — non-blocking veto), **Ask** (side-effecting/escalating — pause for live human or reviewer approval), **Block** (catastrophic — hard-deny, no in-session override, Cedar forbid-override + Sentinel-style hard-mandatory). Design rules from the four shipping agents:
- **Judge actions, not narration.** The optional auto-grant reviewer sees a *stripped* record (user request + actor + tool + resource — no assistant explanation, no tool outputs), so a jailbroken agent cannot talk the guard into approval; it auto-grants only Ask-tier actions, behind the deterministic matrix.
- **Deterministic deny → ask → allow, first-match-wins**, deny-wins-at-any-scope, managed > project > session precedence, so an agent can never widen its own policy ([Claude Code permissions](https://code.claude.com/docs/en/permissions)).
- **Escalation-on-failure**: start least-authority and surface a context-rich Ask at the exact boundary an action hits — fighting fatigue (Claude Code reports ~84% fewer prompts under sandboxing; ~93% of prompts get approved — [Anthropic sandboxing](https://www.anthropic.com/engineering/claude-code-sandboxing)).
- **Typed action card** (MCP form-mode elicitation): actor, verb, resource, blast-radius preview, policy decision path, injection-probe verdict, and grants — Approve-once / Approve-session / Approve-pattern / Deny / Cancel / Takeover (Decline ≠ Cancel).
- **Watch-Mode for the top tier**: a *verified* present human; pause if they disengage. Headless fleets hard-deny anything that would require Watch Mode unless pre-authorized — never silent auto-approve.
- **Fail-closed everywhere**: unmatched → Ask; timeout → does **not** run; Critical → auto-deny.
- **First-class replay events**: every request/grant/override/deny/timeout/revocation is a signed, hash-chained event with triple-identity, params-hash, decision+rule-id, risk tier, taint/injection verdict, and grant scope/lifetime — forkable, non-repudiable, takeover input excluded.
...
12. **Multi-player / non-exclusive computer-use is an explicit open decision** ([notes/open-questions.md](../notes/open-questions.md)). If two principals drive one Sandbox, the human-approver and cross-tenant boundaries need re-derivation. *Accepted:* out of scope until the in/out decision is made.
