...
- **First-party performance numbers now EXIST (RESOLVED 2026-06)** — Shinken's own speed/density corpus is published in [docs/benchmarks/README.md](../benchmarks/README.md). The safe Docker filesystem restore has a rerunnable 0.60 s measurement; the 0.40 s CRIU and 0.118 s live-graft rows are historical pre-hardening artifacts, with the graft disabled and atomic CRIU awaiting a privileged rerun. Separately, one event-loop thread held 3,096 protocol-faithful synthetic sessions (2,320 frames/s ≈ 870 Mbps at 0.93 cores), and act+observe measured 13.4 ms ≈ 14× vs OSWorld's guest server as shipped. Third-party figures keep their **(vendor-published, unverified)** tags; the still-unmeasured boundaries include the sub-ms CoW fork tier and dual-channel WebRTC latency.
