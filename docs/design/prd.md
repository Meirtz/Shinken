...
> **Audience:** product/design maintainers · **Role:** full product requirements · **Source of truth:**
...
*evaluation*, and *trajectory-data capture* on one runtime. It is the runtime that benchmarks
and harnesses plug into, and the product scope is broader than any single benchmark,
browser runner, or remote desktop: Shinken defines the CUA substrate for ACI actions, layered
observation, sandbox capabilities, replay/data, artifact transfer, eval evidence, fleet management,
and human supervision. This PRD enumerates the personas and their top journeys, then the
...
milestones optimize those semantics for fork density, WebRTC/SFU/NVENC, multi-tenant control-plane
scale, cross-substrate scheduling, and cross-OS production fidelity.

---

## 1. Personas & top user journeys

Shinken's north star (D12) is *one* platform with a production runtime and an eval layer layered on top. The first adopters are teams **building and evaluating** computer-use agents — model labs, eval researchers, and RPA builders — for whom the differentiating wedge is **stateful, branchable sandboxes**: name a runnable checkpoint, fork it into N live replicas from one golden state, reset instantly, and resume a suspended session (D1, D5, D7). The event-sourced replay (`.skn`) records what happened on top of that — an audit and RL/SFT trajectory-data byproduct of runs the runtime state makes cheap to spawn. The personas below span both halves of the north star.
...
| P2 | **CUA eval researcher** (who also harvests replay as training data) | Run reproducible, massively-parallel benchmarks by forking N≥5 replicas from one golden snapshot — and mine the recorded trajectories as SFT/RL data as a byproduct | Eval service + Control Panel leaderboard over fork-from-snapshot (D7, D1); `.skn` bundles capture the runs (D5) | D1, D7, D9, D5 |
...
**J1 — Deploy a production agent (P1).** The developer authors an **Operator** (the client-side adapter that drives a Sandbox for a given agent/model and is the human-takeover seam) against the generated SDK. `create_session` claims a Sandbox (local/reference in v0.0.1; warm pool in production, D9); the agent loop drives the typed ACI (D2) over the streaming transport; observation works from screenshots and upgrades to structured a11y/DOM where measured (D3); the Sandbox is provisioned with an explicit capability envelope (D6); and the whole run is recorded as a `.skn` replay (D5). The agent loop stays **provider-agnostic** behind the Operator contract — no vendor lock-in.
...
Observation is **screenshot-first with structured upgrade**. v0.0.1 must work for any GUI through screenshots; normalized accessibility/DOM structure becomes the lower-cost, more stable fast path where apps expose it.

| ID | Requirement | Reconciles |
|----|-------------|-----------|
| FR-OBS-1 | v0.0.1 baseline observation MUST be a **screenshot** (full-screen or focused region) with explicit coordinate space so a screenshot-based GUI agent can run before any app instrumentation exists. | D3 |
...
| FR-STR-6 | The observation tiers MUST map to bandwidth tiers: screenshot baseline for v0.0.1, structured Tier 0 (~20 kbps) where coverage is strong, Set-of-Marks for screenshot-only surfaces, and Tier 2 video for human/live pixel viewing. | D3, D4 |
...
| ID | Requirement | Reconciles |
|----|-------------|-----------|
| NFR-COST-1 | Structured-default observation MUST deliver the ~150× bandwidth win versus H.264 office video where coverage is strong *(≈20 kbps vs ≈3 Mbps — vendor-published, unverified)*; v0.0.1 must measure the blended result instead of assuming it. | D3, D4 |
| NFR-COST-2 | The dominant streaming cost (egress/TURN, not codec) MUST be budgeted explicitly: at 100k concurrent 24×7, ≈$4.9M/mo (H.264 office) vs ≈$0.8M/mo (AV1 screen-content) vs tiny (structured) — *all vendor-derived, unverified, gating a first-party measurement plan*. See [09 Economics](economics-and-build-vs-buy.md). | D4 |
...
- **First-party perf/cost numbers** — PARTIALLY RESOLVED 2026-06: vendor figures above keep their unverified tags, and a first-party benchmark corpus exists ([docs/benchmarks](../benchmarks/README.md)). The Docker filesystem restore's 0.60 s row remains rerunnable; CRIU 0.40 s and live-graft 0.118 s are historical pre-hardening rows, with the graft disabled and atomic CRIU pending a privileged rerun. The 13.4 ms act+observe result is independent. The **a11y-coverage spike** (the load-bearing assumption behind FR-OBS-7) is MEASURED (#2/E5) — verdict: hybrid per-window structured + pixel fallback, D3 stays Provisional (see FR-OBS-7 for the per-surface numbers); games/native-GL remain unmeasured, and cloud-scale density/cost commitments still gate on further measurement.

---

## 5. Success metrics / KPIs

These KPIs gate the phased rollout (see [06 Roadmap](../engineering/roadmap.md)). Targets referencing external figures are anchors pending first-party measurement.
