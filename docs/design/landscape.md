...
The strategic read (reconciled to canon §3): **MATCH** the fork/branch leaders and the model ecosystem and the eval bar; **BEAT** everyone on streaming/bandwidth (every competitor polls screenshots or pushes raw VNC pixels); **DIFFERENTIATE** on event-sourced replay/branching, full cross-platform *desktop* (not browser-only), and an optional GPU-accelerated tier. Per-Sandbox resource scoping (the Capability Manager) rounds out the runtime as plumbing rather than a headline.

**2026-06 amendment — a fifth camp has matured: trainer-side agent/rollout orchestrators.** verl-project/uni-agent (terminal-agent RL: one loop run as 512-way serverless eval, fully-async verl training, and a long-lived chat agent — §2.14), Agentix-Project/Agentix (eval/rollout orchestration over container providers via a remote-Python waist — §2.16), and cua-bench (gym + verl/TRL/Tinker — §2.1) all ship working harnesses that **consume** sandboxes, while xlang-ai/CUA-Gym (§2.15) mass-produces verifiable tasks upstream of any runtime. None owns runtime state (checkpoint/fork) or GUI observation — each pays cold-boot or fresh-sandbox costs per rollout, and the terminal stacks list GUI/multimodal as unchecked roadmap items. Strategic consequence: these are **distribution channels** for Shinken's waist, not rivals — the play is interop (Shinken as the stateful GUI substrate beneath them), not feature race.
...
**Weaknesses.** Snapshot/fork is **split-brain**: `sb.snapshot()` returns an `Image` fed back to `Sandbox.ephemeral()` — docs claim CoW fork "instant on btrfs", 1-5 s typical, with a stateful memory flag for VMs (vendor-published, unverified) — but it is cloud-only (`NotImplementedError` on every local transport) and tied to a closed Kubernetes backend; local tiers degrade to QMP `savevm`/`loadvm` suspend/resume or `docker pause`. Critically, fork is **not wired into cua-bench**: every benchmark reset tears down and recreates the sandbox, so no golden-checkpoint → fork-N → score loop exists anywhere in the stack. Observation remains **pull-based** polled PNG screenshots — a fresh base64-PNG per agent step, no server-push screencast in the SDK (live viewing is offloaded to VNC, or H.265 in the CuaBot demo); the sandbox ACI is flat string verbs with an alias table; the driver deliberately returns checkmark-prose, not typed JSON. A `commandLock` serializes one command at a time per interface — no input batching, RTT per action. Permissions are **coarse**: one API key per VM, no per-action authorization, no FS jail/egress controls in the transport — the safety-check path is literally a TODO. Two overlapping SDK families coexist (legacy `Computer`/`VMProviderFactory` vs. newer `cua_sandbox`).

**Lesson.** ADOPT the layering and three-mode lifecycle and the per-OS handler-factory abstraction wholesale (D2/D8/D10). ADOPT `cua-driver`'s stable element-handle model as the basis of structured observation (D3). **BEAT** them on streaming with the dual-channel ACI (D4) — their explicit gap and Shinken's wedge; their coarse one-key-per-VM model also leaves room for finer per-resource scoping (D6), though that is plumbing, not the wedge. ADOPT the snapshot-returns-`Image` unification and its lifecycle guards into D5's checkpoint API. MINE `cua-driver` as D3 field data: its small-tree heuristic explicitly routes Blender/games/Electron to pixel clicks — production evidence for hybrid-per-window, not structured-default. The surviving wedge vs. cua is harness-integrated, local-first fork (`run_eval_forked`) plus the typed wire contract and server-push streaming they lack (D1/D2/D4).
...
**Lesson.** **MATCH** (non-negotiable): the exact tool schema, version/beta-flag grammar, coordinate space, and the resize math (so the pixels sent equal the pixels the model reasons over) — via a version-pinned `AnthropicComputerAdapter` (D2). **BEAT** the screenshot-per-step model and the stock-VNC wire (D3/D4 — this is literally what Shinken exists to win). ADD what Anthropic lacks: durable event-sourced replay (D5) and per-Sandbox resource scoping (D6), where credential directories are scoped by default through `fs.scope` rather than left in-scope.
...
**Strengths.** Together with Anthropic, it defines the small stable action vocabulary Shinken's ACI must speak natively, the **batched `actions[]` per turn** pattern (cuts round-trips), and a machine-readable confirmation seam (`pending_safety_checks`) plus a takeover/watch mode so secrets never enter the observation stream.
...
**Weaknesses.** Both default proxies allowlist by **client-supplied hostname without TLS inspection**, so the egress scope is looser than it looks. Claude Code's default `denyRead` still leaves `~/.aws/credentials` and `~/.ssh` in scope. Bash argument-constraining rules are fragile (`Bash(curl http://github.com/ *)` is easily worked around). Claude Code computer-use runs on the **real desktop, not sandboxed** — a looser isolation boundary than its Bash path. Codex's `bwrap.rs` is 2700+ lines (FS-policy translation is genuinely hard).

**Lesson.** ADOPT the two-layer UI split (a "what it CAN touch" OS-sandbox layer vs. a "when to ASK" approval-policy layer), the strictest-wins decision merge, ordered path-specificity FS policy, the escalation prompt shape (justification + minimal grant + scope choices), and the out-of-sandbox egress proxy reached over a Unix socket with an OS backstop (D6). ADOPT Codex's HTTP-method allowlist (GET/HEAD/OPTIONS) and offer an optional TLS-terminating mode where deeper egress scoping is wanted. Improve on their gaps: scope credential dirs by default, run computer-use inside the sandbox, and tighten the egress-scope check at the proxy (see the [08 Isolation & capability note](threat-model.md)).
...
### 2.14 verl-project/uni-agent — the terminal-agent RL incumbent

**What.** A unified model+tool+env loop for bash agents over SWE-ReX's six deployment backends; three consumers run on one stack — parallel inference at 512-way serverless concurrency, verl `AgentLoop` training with token-exact trajectories (GRPO, MoE-router-replay), and an IM chat agent. Source: <https://github.com/verl-project/uni-agent> (surveyed 2026-06-10).

**Strengths.** Production validation of the consumers-over-one-waist model (the same shape as Shinken's narrow-waist runtime). Per-sample env+reward config carried in dataset rows; in-sandbox `RewardSpec`s with gold-patch CI dry-runs; a three-level never-crash outcome taxonomy (`ToolStatus` / `exit_reason` / `traj_masked`).

**Weaknesses.** One action verb (bash-in-session); **zero GUI/pixels/a11y** (roadmap-unchecked). **Zero runtime state** — no reset/snapshot/fork, so GRPO with `n_resp_per_prompt=8` pays 8 cold boots of the same image per prompt, mitigated only by a creation semaphore; their own docs concede rollout capacity is often the first bottleneck. No capability gate on model-generated bash.

**Lesson.** Make verl the reference train consumer (#223): Shinken `Trajectory` ↔ `AgentLoopOutput` convertibility plus a swerex-protocol shim, so a Shinken sandbox is a drop-in deployment backend whose `start()` forks from a golden checkpoint. Their GUI gap is Shinken's lane (D1/D7/D12).

### 2.15 xlang-ai/CUA-Gym (+ CUA-Gym-Hub) — the verifiable-task factory upstream of the runtime

**What.** A coding-agent pipeline that co-generates `(instruction, executable setup, programmatic reward)` tuples in a generator/critic loop, machine-validated by `reward(golden_env)==1.0` AND `reward(initial_env)==0.0` on two isolated cloud VMs per task; 32,112 OSWorld-format bundles released on HuggingFace. The Hub is 98 mock React apps behind one HTTP state API (`set`/`set_current`/`reset`, server-computed `state_diff`, per-session `?sid=` isolation). Runs on the OSWorld Flask protocol. Sources: <https://github.com/xlang-ai/CUA-Gym>, <https://github.com/xlang-ai/CUA-Gym-Hub> (surveyed 2026-06-10); paper: <https://arxiv.org/abs/2605.25624>.

**Strengths.** The content + verifier-governance story: a locked LLM-judge module, a ≥60%-programmatic rule, and a majority-vote critic calibrated against a labeled meta-benchmark ("verify the verifier").

**Weaknesses.** Desktop reset is **two fresh cloud VMs × ~10 min per task**, concurrency 3 "limited by VM budget" — isolation bought with a second VM where a fork would do. The guest protocol stays unauthenticated HTTP on `:5000`. a11y is available but never used (verification = file/app state + a budgeted vision judge).

**Lesson.** The archetype customer for runtime state: golden-checkpoint-after-setup + fork-2 replaces the dual-VM tax (D1/D7). ADOPT the golden/initial double-test as `run_eval_forked`'s verifier-validation mode; support its bundles as a second eval TaskSource. Scope the fork claim honestly: Hub-style app-level state injection solves stateless-web reset in milliseconds, so fork is decisive on **stateful** desktop/OS/app-memory environments and composes with sid-isolation rather than competing.

### 2.16 Agentix-Project/Agentix — the layer-above orchestrator archetype

**What.** MIT framework (PyPI: `agentixx`) for code/terminal-agent eval + rollout collection, built on two primitives — a content-addressed runtime bundle overlaid read-only onto any task image, and `sandbox.remote(importable-python-fn)` over Socket.IO/pickle — plus Docker/Podman/Apptainer providers, a typed batch runner, and an LLM-proxy tunnel keeping credentials host-side while stamping per-rollout session ids. Source: <https://github.com/Agentix-Project/Agentix> (surveyed 2026-06-10).

**Strengths.** The cleanest shipped narrow-waist orchestration in the field: reconnect-safe resume/ack event delivery; typed worker-death errors (the -9 SIGKILL/OOM idiom); an entry-point plugin registry with duplicate-name conflict errors and provenance reporting.

**Weaknesses.** Python-only pickle waist — cannot speak to a non-Python guest. No GUI. No checkpoint/fork: scoring uses a second fresh sandbox, and "Checkpoint/partial rollout … then fork" is a one-line roadmap "Later" bullet with no substrate.
...
|---|---|---|---|
| **Cross-platform computer surfaces** | Built D15 adapters cover Linux/macOS/Windows/browser API paths through one capability-negotiated ACI; native Linux is CI-backed, native macOS capture+input v1 has local proof, cua/mcp/e2b external live parity is pending, and native Windows/Wayland plus full macOS AX remain roadmap (D10/D15) | cua: macOS+Linux+Windows/Android (closest). E2B/Morph: Linux only. Browser cohort: Chromium only. | **Adapter/API reach matches; external live and native parity incomplete** |
...
| **Reset / fork speed** | MATCH Morph: target <30 ms VMM restore + sub-ms CoW fork on Linux tier (MAP_PRIVATE + userfaultfd + warm pools); GPU/Win/mac tiers snapshot-light (D1) | Morph: P99 ~1.3 ms fork (best). E2B: ~150 ms restore. cua: cloud-only snapshot + CoW fork ("instant on btrfs", 1-5 s typical — vendor-published, unverified; `NotImplementedError` on local transports; closed backend); local QMP `savevm`/`loadvm`; NOT integrated into cua-bench (resets recreate the sandbox). uni-agent / Agentix / CUA-Gym: none — cold boot or fresh VM per rollout. | **MATCH** (Linux); honest **degrade** on GPU/Win/mac — the unclaimed ground is harness-integrated, local-first fork (`run_eval_forked`) |
...
| **Replay / branching** | DIFFERENTIATE: one event-sourced logical-clock timeline (the live stream IS the replay log) + bisected env/agent snapshots + immutable checkpoint DAG; scrubbable, forkable, greppable (D5) | OSWorld: `traj.jsonl` + `recording.mp4` only. Morph: VM-level branch, no agent-trajectory timeline. cua: HF-Datasets trajectory traces + a viewer (cua-bench), no event-sourced branchable timeline; E2B: none. | **DIFFERENTIATE** — largely greenfield |
| **Resource scoping** | Plumbing: 3-layer capability model — Cedar decision + ocap caretaker/membrane + OS enforcement; 8 capability classes, 4 scope tiers, live HITL card (D6) — scopes each Sandbox to the resources its task needs | OpenAI: `pending_safety_checks`. Anthropic: classifiers + HITL. Codex/Claude Code: OS sandbox + egress allowlist. cua/E2B/OSWorld: none / TODO. | **runtime plumbing** — finer-grained than per-command coding tools, but not a headline |
| **AI-native ACI** | Native streaming ACI: small typed verb set + screenshot-first baseline, a11y/SoM structured upgrade with stable element refs, video on demand; version-pinned bidirectional adapters (Anthropic/OpenAI/UI-TARS/computer_13) as the only model-facing surface (D2/D3) | Anthropic `computer_2025xxxx` + bash + text_editor. OpenAI `computer_call` + `actions[]`. cua: typed WS dispatch. | **MATCH** the schemas, **DIFFERENTIATE** by making screenshots usable first and structure cheaper/stabler where available |
| **Eval support** | MATCH + reproduce: built-in OSWorld-Verified / WindowsAgentArena / AndroidWorld / WebArena conformance, deterministic setup, N≥5 CoW replicas → pass@k/pass^k + CIs, graders as tested artifacts (D7) | OSWorld-Verified: the de-facto bar (369 tasks, AWS ~50×, ~1h). HUD: hosts it as MCP RL envs. cua-bench: real gym + RL harness (oracle solvers, verl/TRL/Tinker integrations, worker pools). CUA-Gym: 32k oracle-validated OSWorld-format RLVR tasks. | **MATCH** + inversion (thin orchestration on the runtime, typed verifier DAG) |
...
**Bandwidth economics (the BEAT axis).** Structured ≈ 20 kbps vs. H.264 office content ~3 Mbps ≈ **~150×** cheaper; AV1 screen-content ≈ ~40% below H.264 (vendor-published, unverified). At N=100k concurrent 24×7, egress runs ≈ $4.86M/mo (H.264 office) vs. ≈ $814k (AV1-SCC busy) vs. *tiny* (structured) — see [09 Economics & Build-vs-Buy](economics-and-build-vs-buy.md) and <https://developer.nvidia.com/blog/improving-video-quality-and-performance-with-av1-and-nvidia-ada-lovelace-architecture/>. This is why Shinken invests in structured-default paths where coverage is strong, while keeping screenshots as the universal baseline.
...
**Choice (D6):** 3-layer Sandbox Capability Manager — runtime plumbing that scopes each Sandbox to the resources its task needs. **8 resource classes** (`net.egress, fs.scope, clipboard, gpu, install.privileged/sudo`, `persistence, credentials, peripheral/OS automation`), each scoped. Ordinary in-sandbox actions run without prompts; boundary powers are granted, narrowed, revoked, and replayed. Egress = out-of-VM proxy (deny-by-default, scoped-domain, optional TLS-terminating mode); secrets brokered via Vault/KMS + proxy header-injection — the model never sees plaintext. See the [08 Isolation & capability note](threat-model.md) and [`notes/permissions.md`](../../notes/permissions.md).
...
**Observation (D3):** screenshot-first with layered structured upgrades. Rung 0 = screenshots /
focused regions that work everywhere. Rung 1 = normalized cross-OS a11y/DOM tree diff
(AT-SPI/UIA/AX/CDP) → one `Element{ref, role, name, value, states, bbox, source}` schema with
stable per-session refs (~6× token savings, ~25k vs ~150k tokens/task — vendor-published,
...
Shinken is **not** trying to out-breadth cua on day-one backend count, out-fork Morph on the raw
