...
- **Code-as-action** (`exec`/`bash`/`edit`) is a separate, **off-by-default capability class** that routes through the **`tool_runner` policy boundary** (D6) — never the open RCE primitive OSWorld ships.
  ...
  **Sub-decision — superseded 2026-06 after #56:** typed `exec` is now an ACI wire verb
  (argv by default, explicit shell opt-in, bounded output/streaming, timeout group-kill, and
  gateway audit); `launch_app` is also typed on the wire. `put_file`/`get_file` remain
  substrate-side artifact channels, and PTY remains reserved/unsupported. CUA-Gym and
  swerex prefer the in-band `exec` capability when negotiated and retain an out-of-band
  fallback for older runtimes. This keeps code execution explicit and auditable without
  pretending file transfer is an ACI action.
