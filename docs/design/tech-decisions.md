...
| **D5** | Runtime state + replay = event stream + snapshots + checkpoint DAG; `.skn` evidence bundle | Accepted | Immutable checkpoint DAG (snapshot/checkpoint/fork/resume) with append-only `events.jsonl` evidence ledger; reset and branch are one primitive |
